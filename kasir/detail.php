...
if (isset($_POST['simpan'])) {
  ...
  $kode_update = $kode;
  $queryCek = "SELECT * FROM tb_transaksi WHERE kode_invoice = '$kode_update'";
  ...
  if (mysqli_num_rows($execCek) == 1) {
    ...
    if ($execTambah) {
      echo "<script>alert('Update Berhasil'); window.location.href='detail.php?idtransaksi=$idTransaksi&kode=$kode';</script>";
    }
    } else {
